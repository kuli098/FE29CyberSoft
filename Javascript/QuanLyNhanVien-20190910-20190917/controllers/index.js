...
function taoBang(mangNV) {
  ...
  for (var i = 0; i < mangNV.length; i++) {
    ...
    dsTheTR += `
        <tr>
          <td><input type= "checkbox" class="ckbXoaNhanVien" index="${i}" manv="${nv.maNV}" /></td>
          <td>${nv.maNV}</td>
          <td>${nv.hoTen}</td>
          <td>${nv.email}</td>
          <td>${nv.ngaySinh}</td>
...
//yêu cầu thực hiện tính năng xóa phần tử dựa vào input checkbox
document.getElementById('checkAll').onclick = function(){
  // lấy giá trị input #checkAll khi người dùng click vào
  var ckAllValue = document.getElementById('checkAll').checked;
  // Dom checkbox còn lại => set gái trị cho thuộc tính checked = ckAllValue
  var lstCkbXoaNV = document.getElementsByClassName('ckbXoaNhanVien');
  for(var i =0; i < lstCkbXoaNV.length; i++){
    lstCkbXoaNV[i].checked = ckAllValue;
  }
}

document.getElementById('btnLstXoaNhanVien').onclick = function(){
  //duyệt những nhân viên nào được checked
  var lstCkbNhanVien = document.getElementsByClassName('ckbXoaNhanVien');
  for(var i = lstCkbNhanVien.length-1; i >= 0; i--){
    // lấy ra từng check box
    var ckbNV = lstCkbNhanVien[i];
    if(ckbNV.checked){
      var index = ckbNV.toggleAttribute("index");
